...
Configures Stride integration for Auggie (Augment Code).

Integration Type: Hybrid (root config + slash commands)
Files Created:
- AUGGIE.md (root config with managed block)
- .auggie/prompts/stride-*.toml (9 workflow commands)
"""
...
class AuggieConfigurator(ToolConfigurator):
    """Configurator for Auggie."""
    ...
    def configure(self, project_path: Path) -> ConfigResult:
        """Create Auggie integration files."""
        messages = []
        files_created = []
