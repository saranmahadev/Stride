...
Configures Stride integration for CodeBuddy Code.

Integration Type: Root Config Only
Files Created:
- CODEBUDDY.md (root config with managed block)
"""
...
class CodeBuddyConfigurator(ToolConfigurator):
    """Configurator for CodeBuddy."""
    ...
    def configure(self, project_path: Path) -> ConfigResult:
        """Create CodeBuddy root config file."""
        messages = []
        files_created = []
    ...
    def validate(self, project_path: Path) -> ValidationResult:
        """Validate CodeBuddy integration."""
        issues = []
        warnings = []
