"""
Factory Droid Configurator

Configures Stride integration for Factory Droid.

Integration Type: Hybrid (root config + slash commands)
Files Created:
- FACTORY.md (root config with managed block)
- .factory/prompts/stride-*.toml (9 workflow commands)
"""
...
class FactoryConfigurator(ToolConfigurator):
    """Configurator for Factory Droid."""
    ...
    def configure(self, project_path: Path) -> ConfigResult:
        """Create Factory integration files."""
        messages = []
        files_created = []
