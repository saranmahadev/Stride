...
Configures Stride integration for Codex.

Integration Type: Hybrid (root config + slash commands)
Files Created:
- CODEX.md (root config with managed block)
- .codex/prompts/stride-*.toml (9 workflow commands)
"""
...
class CodexConfigurator(ToolConfigurator):
    """Configurator for Codex."""
    ...
    def configure(self, project_path: Path) -> ConfigResult:
        """Create Codex integration files."""
        messages = []
        files_created = []
