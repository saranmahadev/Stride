...
Configures Stride integration for OpenCode.

Integration Type: Hybrid (root config + slash commands)
Files Created:
- OPENCODE.md (root config with managed block)
- .opencode/prompts/stride-*.toml (9 workflow commands)
"""
...
class OpenCodeConfigurator(ToolConfigurator):
    ...
    def configure(self, project_path: Path) -> ConfigResult:
        """Create OpenCode integration files."""
        messages = []
        files_created = []
